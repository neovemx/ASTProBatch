...
{
    public class LogExecutionDelayChartViewModel : BaseViewModel
    {
        #region Atributes
        ...
        #region Constructor
        public LogExecutionDelayChartViewModel(bool IsReload, List<LogExecutionDelayResult> LogExecutionDelayResult)
        {
            if (IsReload)
            {
                this.Zoom = 230;
                ...
                {
                    TimeSpan.TryParse(logExecutionDelayResults.DelayTime, out TimeSpan time);
                    entries.Add(new ChartEntry((float)time.TotalMilliseconds)
                    {
                        Label = logExecutionDelayResults.Command,
                        ValueLabel = logExecutionDelayResults.DelayTime,
